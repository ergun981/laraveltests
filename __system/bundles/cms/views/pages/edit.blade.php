...
<div class="span6">
	...
	<div class="w-box w-box-red">
		...
		<div class="w-box-content">
			<form class="form-horizontal" method="post" action="{{ action('cms::pages@update_langs', array($page->id))}}">
						...
						<div class="tab-content">
							...
							<div id="lang-{{ $lang->id }}" class="tab-pane <?php if($is_first) echo 'active'; $is_first= false; ?>">
								...
								<div class="control-group">
									...
									<div class="controls">
										<input <?php if(!is_null($pagelang->url)){?> disabled <?php } ?> name="{{ '_'.$lang->id }}[url]" type="text" id="url{{ $lang->id }}" placeholder="URL" value="{{$pagelang->url}}">
									</div>
								</div>
	...
	<div class="w-box w-box-red">
		<div class="w-box-header">
			<h4>Makaleler</h4>
			<i class="icsw16-create-write icsw16-white pull-left"></i>
			<a href="{{action('cms::articles@new', array($page->id))}}" class="btn btn-small btn-primary pull-right" title="Article Ekle"><i class="icon-plus icon-white"></i>Makale Ekle</a>
		</div>
		<div class="w-box-content">
					...
					<table class="table table-vam table-striped dataTable" aria-describedby="dt_gal_info">
						...
						<tbody role="alert" aria-live="polite" aria-relevant="all">
							...
							<tr class="odd">
								...
								<td class=" ">
									<div class="btn-group">
										<a title="Düzenle" class="btn btn-mini" href="{{ action('cms::articles@edit', array($page_article->article->id))}}"><i class="icon-pencil"></i></a>
										<a title="Sil" class="btn btn-mini" href="{{action('cms::articles@delete', array($page_article->article->id))}}"><i class="icon-trash"></i></a>
									</div>
								</td>
							</tr>
							@endforeach
...
<div class="span3">
	<div class="w-box w-box-red">
		...
		<div class="w-box-content">
			<form class="form-horizontal" method="post" action="{{action('cms::pages@update_details', array($page->id))}}">
