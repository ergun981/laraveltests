...
class Cms_Pages_Controller extends CmsBase_Controller {
	...
	public function __construct(){
		...
		$pages_dropdown = array_merge(array(-1 => '---'), Page_Lang::where('lang_id','=',1)->lists('title', 'id'));
	...
	public function get_new() {
		...
		$data = array();
		return view('cms::pages.new', $data);
	}
	...
	public function post_new() {
		...
		if (!$errors) {
			...
			*/
			...
			Session::flash('status_success', 'Yeni Sayfa Eklendi!' . $page->id);

			return Redirect::to_action('cardea::dashboard');
		} else {
			return Redirect::to_action('cms::pages@new')
			->with_errors($errors)
			->with_input();
		}
	}
	...
	public function get_edit($id = NULL) {
		...
		$page = Page::with(array('page_langs','page_articles','page_articles.article','page_article.article.article_langs'=>function($query){
	...
	public function post_update_langs($page_id = NULL) {
		...
		foreach ($posted as $key => $value) {
			...
			$pagelang = Page_Lang::where('page_id', '=', $page_id)
