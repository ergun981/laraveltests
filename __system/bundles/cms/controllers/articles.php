...
class Cms_Articles_Controller extends CmsBase_Controller {
	
	public function get_index() {
		$articles = Article::with(array('page_articles','page_articles.page','page_articles.page.page_langs'=>function($query){
		...
		return view('cms::articles.list', $data);
	}
...
{
    $form = Input::all();
    $article = Article::find($form['article_id']);
        ...
        foreach ($form['lang'] as $id=>$lang)
        {
            ...
            {
                $article_lang = Article_Lang::find($lang['article_lang_id']);
            }
            else 
            {
                $article_lang = new Article_Lang;
        ...
        $page_article = Page_Article::where('article_id','=',$article->id)->first();
        $page_article->is_online = isset($form['is_online']) ? $form['is_online'] : '0';
        $page_article->save();
    ...
    public function get_delete($id = null)
    {
        ...
        return Redirect::to_action('cms::articles@index');
    }
